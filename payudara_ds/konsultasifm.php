<html>
<title>Sispak Kanker Payudara|DS</title>
<body>
<?php include "koneksi2.php"; ?>
<table width="750" border="1" align="center">
  <tr>
    <td width="786"><h3>PROSES DETEKSI DINI PENYAKIT KANKER PAYUDARA</h3><strong>Pilihlah Gejala Yang Anda Alami..!</strong></td>
  </tr>
  <tr>
    <td>
<form method="post" >
...
?><br>
<center><input type="submit" value="Mulai Deteksi"></center>
</form>
...
if(isset($_POST['evidence'])){
	if(count($_POST['evidence'])<2){
		...
		foreach($arrKDGejalaSelect as $kdGSelect){ 
			echo $kdGSelect." | ";
			$queryG=mysql_query("SELECT * FROM tb_gejala WHERE id='$kdGSelect' "); $dataG=mysql_fetch_array($queryG); echo $dataG['gejala']."<br>";
			}
		echo "</div>";
		...
		foreach($evidence as $kdgejala){
			...
			echo "<td>$no</td>";
			echo "<td>"; 
			$queryG=mysql_query("SELECT * FROM tb_gejala WHERE id='$kdgejala[2]' "); $dataG=mysql_fetch_array($queryG); 
				...
				for($x=0;$x<2;$x++){
					if(!($y==$m-1 && $x==1)){
						...
						if(empty($vw)){
							...
							$k="{}"; 
							echo " $k<br>";
							echo $nilaiX1Y1 =$densitas1[$x][1]*$densitas2[$y][1];
							echo "</span>"; 
						}else{
							//echo "<td rowspan='2'>";
